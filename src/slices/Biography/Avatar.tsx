...
export default function Avatar({ image, className }: AvatarProps) {
	...
	useEffect(() => {
		let ctx = gsap.context(() => {
			...
			window.onmousemove = (e) => {
				...
				gsap
					.timeline({
						defaults: { dudraion: 0.5, overwrite: 'auto', ease: 'power3.Out' },
					})
					.to(
						'.avatar',
						{
	...
	return (
		<div ref={component} className={clsx('relative h-full w-full', className)}>
			<div className='avatar aspect-auto overflow-hidden rounded-3xl border-2 border-slate-700 opacity-0'>
				<PrismicNextImage
					field={image}
					className='avtar-image h-full w-full object-fill'
					imgixParams={{ q: 90 }}
				/>
